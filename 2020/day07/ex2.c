...
int nb_sub_bags(int bag_id) {
  int sub_bag_id;
  int total = 0;

  for (sub_bag_id = 0; sub_bag_id < MAX_SUB_BAGS_LENGTH && bag_contents[bag_id][sub_bag_id].nb_bags > 0; sub_bag_id++) {
    int nb_bags_of_type = bag_contents[bag_id][sub_bag_id].nb_bags;
    int sub_type_id = bag_contents[bag_id][sub_bag_id].bag_type;
    int nb_sub_bags_in_type = nb_sub_bags(sub_type_id);
    int nb_bags = nb_bags_of_type * (nb_sub_bags_in_type + 1);
    total += nb_bags;
  }
